...
// Check if a name valid (for Lan setting - domain)
function isValidName2(name) {
   var i = 0;	
   
   for ( i = 0; i < name.length; i++ ) {
      if ( isCharUnsafe_domain(name.charAt(i)) == true )
         return false;
   }
...
// same as is isCharUnsafe but allow spaces
function isCharUnsafe_domain(compareChar) {
   var unsafeString = "\"%\\[]{}:;></!$^*()`\+\,='#&\t ";
   if ( unsafeString.indexOf(compareChar) == -1)
      return false; // found no unsafe chars, return false
   else
...
function isValidIpAddress(address) 
{
   ...
   for (i = 0; i < 4; i++) 
   {	   
      if (isNaN(addrParts[i]) || addrParts[i] =="" || addrParts[i] ==" ")
         return false;
	  ...
	  if(i==0 && num==0)	//check if the first bit is 0
	  	 return false;
...
function isValidMacAddress(address) {
   ...
   if ( address == 'ff:ff:ff:ff:ff:ff' || address == 'FF:FF:FF:FF:FF:FF') return false;
...
function isValidNum(val)
{
   ...
   for ( i = 0; i < val.length; i++ ) 
   {
         if ( unsafe.indexOf(val.charAt(i)) != -1 ) 
            ret = false;
   }
...
function isValidDNSIpAddress(address) 
{
   var cnt=0;  //for checking whether ip address > 224.0.0.0 or not
   ...
   {	   
	  ...
	  if( (i==0 && num ==0) || (i==0 && num ==255) || (i==0 && num > 224))
	  	return false;
