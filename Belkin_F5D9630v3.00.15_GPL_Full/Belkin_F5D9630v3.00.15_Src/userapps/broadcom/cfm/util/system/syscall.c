...
#if FOR_BELKIN
/* Joshua */
static struct in_addr adminAddr;


struct in_addr admin_ip()
{
  return adminAddr;
}

/* Joshua */
int isAdminIP(struct in_addr clntAddr)
{
   int ret = 0;
   
   if(adminAddr.s_addr == 0)
   {   
   	  //printf("adminAddr is 0\n");
        ret = 1;
   }
   else
   {
      if ( memcmp(&(clntAddr.s_addr),&(adminAddr.s_addr),sizeof(in_addr_t)) == 0  )
     {     
		ret = 1;
     }
   }	  
...
int isFirstLogIP(struct in_addr clntAddr)
{
   int ret = 0;
   
   if(adminAddr.s_addr == 0)
   {   
   	  adminAddr.s_addr = clntAddr.s_addr; 
      ret = 1;
   }
   else{
      if ( memcmp(&(clntAddr.s_addr),&(adminAddr.s_addr),sizeof(in_addr_t)) == 0  )
...
int bcmCreateDhcpCfg(char *ipAddr, char *mask,
                     ...
                     int protocol, int enblNat, int enblFirewall, char *dhcpDomain) {
#else
int bcmCreateDhcpCfg(char *ipAddr, char *mask,
                     char *addrStart, char *addrEnd,
                     char *dns1, char *dns2, int leasedTime,
...
//printf("\nalex : in bcmDisplayLed with status = %d\n\n",status);
#if FOR_BELKIN //Add by Alex for Internet LED
   char buf[256];
   int ret;
   int stsMntr = MNTR_STS_ADSL_DOWN;
#endif
   ...
   case MNTR_STS_PPP_DOWN:
      ...
      /* sysLedCtrl(kLedPPP, kLedStateFail); */
      sysLedCtrl(kLedPPP, kLedStateOff);   //Add by Alex
      break;
   case MNTR_STS_OK:
      /* this means ADSL and PPP are up */
      //Modify by Alex : reverse the meaning of Internet led	  	    
#if FOR_BELKIN
     //ret = bcmGetSystemStatus(buf, 256);
     ret = bcmGetAdslStatus();

     //printf("alex : ret = %d\n\n",ret);
     if(ret == MNTR_STS_ADSL_DOWN)
      		sysLedCtrl(kLedWanData, kLedStateOff);	  
     else
      		sysLedCtrl(kLedWanData, kLedStateOn);	  
#endif
...
#if FOR_BELKIN
   /* do not remove the modules, otherwise the system will hang */
#else
...
*/
      ...
      if(BcmDb_isRemoteAdminEnabled())
      {
         while ( bcmRemoveIpTableRule(device, "nat", "remote_management") == TRUE )
            ;
      }
...
void bcmRemoveModules(int lanIf)
{
   ...
   while (modList[i] != NULL)
   {
      ...
      {
#if FOR_BELKIN // do not try to remove iptable_nat module, otherwise the system will hang
         if ((strcmp(modList[i], "iptable_nat") != 0) || (strcmp(modList[i], "iptable_filter") != 0))
#else
...
void bcmRemoveTrafficControlRules(UINT32 portId, UINT16 vpi, UINT16 vci, UINT16 conId, UINT8 protocol) {
    ...
    ifc[0] = '\0';
    snprintf(ifc, 16, "ppp_%d_%d_%d_%d", portId, vpi, vci, conId);
...
void bcmAddTrafficControlRules(UINT32 portId, UINT16 vpi, UINT16 vci, UINT16 conId, UINT8 protocol) {
    ...
    ifc[0] = '\0';
    sprintf(ifc, "ppp_%d_%d_%d_%d", portId, vpi, vci, conId);
...
void bcmGetBridgeInterfaceInfo(char *varValue, char *skip) {
   char info[IFC_MEDIUM_LEN], interface[IFC_TINY_LEN];

   int i,length,token_len,index = 0;
   char name[SYS_CMD_LEN],string1[SYS_CMD_LEN],string2[SYS_CMD_LEN];
   char groupName[IFC_TINY_LEN];

   varValue[0] = info[0] = interface[0] = '\0';
   ...
   while ( index < PORT_MAP_MAX_ENTRY) {
      ...
      token_len = strlen(skip);
      length = strlen(name);
      for(i=0;i<=(length-token_len);i++) {
        if(!strncmp(name+i, skip, token_len)) break;
      }
      printf("token_len %d name_len %d place %d\n", token_len, length, i);
      if(i==0)
        sprintf(info, "%s|", name+token_len+1);
      else if (i==(length-token_len))
        strncpy(info, name, i);
      else {
        strncpy(string1, name, i);
        sprintf(string2, "%s|", name+i+token_len+1);
        sprintf(info, "%s%s", string1, string2);
      }
      //sprintf(info, "%s|", name);
      if ( info[0] != '\0' )
         strcat(varValue, info);
      index++;
      memset(name, '\0', SYS_CMD_LEN);
   }
