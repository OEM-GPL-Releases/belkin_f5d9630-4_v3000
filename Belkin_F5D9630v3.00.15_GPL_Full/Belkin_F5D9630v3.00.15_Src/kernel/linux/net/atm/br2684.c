...
#if defined(ATM_CACHE_SMARTFLUSH)
                /* for smartflush, need to flush a few extra bytes as we are not going to flush when recycling buffers */
                pAb->pWholeFlushStart = skb->head;
                pAb->pWholeFlushEnd = skb->end;
#endif                    
...
#if defined(ATM_CACHE_SMARTFLUSH)
                    if (pDp->ucFlags & ATMDATA_FROM_XMIT_CLEAN) {
                        skb->retfreeq_flags |= SKB_FROM_XMIT_CLEAN; /* buffer is clean now - nothing is in cache */
                    }
#endif
                    if (devext) {
        	                devext->stats.tx_packets++;
    ...
    int nFlag )
{
    if( nFlag & RETFREEQ_SKB )
    {
        ...
        if (skb->retfreeq_flags & SKB_FROM_XMIT_CLEAN) { /* pass flag to atm buffer */
            pDp->ucFlags |= ATMDATA_FROM_XMIT_CLEAN;
        }
#endif            
        (*pDp->pFnFreeDataParms)(pDp);
    }    
