...
//Alex : modify for Belkin log style
    if((strcmp(info->prefix, "SYNFLOOD") == 0) || (strcmp(info->prefix, "SYN FLOOD ATTACK") == 0) || 
	   (strcmp(info->prefix, "PING") == 0) || (strcmp(info->prefix, "PING OF DEATH ATTACK") == 0) ||
	   (strcmp(info->prefix, "ICMPFLOOD") == 0) || (strcmp(info->prefix, "ICMP FLOOD ATTACK") == 0) || 
	   (strcmp(info->prefix, "PORTSCANNER") == 0) || (strcmp(info->prefix, "PORT SCANNER ATTACK") == 0) ||
	   (strcmp(info->prefix, "LANDATTACK") == 0)|| (strcmp(info->prefix, "LAND ATTACK") == 0))	 
	{
		printk("detected from %u.%u.%u.%u. ", NIPQUAD(iph.saddr));
		//return;
	}
	else if(strcmp(info->prefix, "RemoteAccess:") == 0)
	{
		printk("%u.%u.%u.%u attempting to remotely manage the Router", NIPQUAD(iph.saddr));	
		return;
	}
	else
		printk("from %u.%u.%u.%u. ", NIPQUAD(iph.saddr));
	


//Alex : modify for Belkin log sytle
	switch (iph.protocol) {
		case IPPROTO_TCP:{
			struct tcphdr tcph;
			...
			printk(" Source port is %u, and destination port is %u which use the TCP protocol.",ntohs(tcph.source), ntohs(tcph.dest));
		...
		case IPPROTO_UDP:{	
			...
			/* Max length: 20 "SPT=65535 DPT=65535 " */
			printk(" Source port is %u, and destination port is %u which use the UDP protocol.",ntohs(udph.source), ntohs(udph.dest),ntohs(udph.len));
			
			break;
		}
		case IPPROTO_ICMP: {
			printk("Use ICMP protocol");			
			break;
		}
		case IPPROTO_AH:{
			printk("Use AH protocol");
			break;
		}
		case IPPROTO_ESP:{
			printk("Use ESP protocol");
			break;
		}
		default:
			printk("Use %u protocol", iph.protocol);			
		}
...
//Alex : modify for Belkin log style
	...
	printk("%sIN=%s OUT=%s ",
	       ...
	       out ? out->name : "");
#else
	printk("%s ",prefix == NULL ? loginfo->prefix : prefix);
#endif
